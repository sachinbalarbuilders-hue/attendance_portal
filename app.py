    ...
    def reset_password(self, email, new_password):
        ...
        if email in self.EMPLOYEE_DB:
            ...
            if email.endswith('@gmail.com'):
                ...
                # Try to find matching employee name from database first
                employees = db.get_employees()
                employee_name = None
                for emp_name in employees:
                    cleaned_emp_name = self.clean_employee_name(emp_name)
                ...
                # If not found in database (first-time password change), create a generic name
                if not employee_name:
                    # Convert email_name back to a readable format
                    employee_name = email_name.replace('(', ' (').replace(')', ')').title()
                    print(f"Creating new employee account for first-time password change: {email}")
...
def forgot_password_send_otp():
    ...
    try:
        ...
        # Store OTP in database with the stored email
        if db.store_otp(email, otp_code, stored_email):
            # Send OTP email to the stored email
            if email_service.send_otp_email(stored_email, otp_code, 'Employee'):
                return jsonify({
                    'success': True, 
                    'message': f'Password reset code sent to {stored_email}',
                    'email': stored_email
                })
            else:
                return jsonify({'success': False, 'message': 'Failed to send password reset code'})
        else:
            return jsonify({'success': False, 'message': 'Failed to generate password reset code'})
            
    except Exception as e:
        print(f"Error sending forgot password OTP: {e}")
        return jsonify({'success': False, 'message': 'An error occurred while sending password reset code'})
...
def forgot_password_verify_otp():
    ...
    email = data.get('email')  # We need to get email from frontend
    ...
    try:
        # Verify OTP
        if db.verify_otp(email, otp_code):
            # Store forgot password verification in session
            session['forgot_password_verified'] = True
            session['forgot_password_email'] = email
            session['forgot_password_verified_at'] = time.time()
...
def forgot_password_reset():
    """Reset password after OTP verification for forgot password"""
    # Check if forgot password was verified in session
    if not session.get('forgot_password_verified'):
        return jsonify({'success': False, 'message': 'OTP verification required'})
    
    # Check if forgot password verification is not too old (5 minutes)
    verified_at = session.get('forgot_password_verified_at', 0)
    if time.time() - verified_at > 300:  # 5 minutes
        session.pop('forgot_password_verified', None)
        session.pop('forgot_password_email', None)
        session.pop('forgot_password_verified_at', None)
        return jsonify({'success': False, 'message': 'OTP verification expired. Please verify OTP again.'})
    ...
    try:
        ...
        if employee_db.reset_password(email, new_password):
            ...
            db.log_password_change(
                ...
                current_password=new_password,
                changed_by='forgot_password'
            )
            
            # Clear forgot password verification from session
            session.pop('forgot_password_verified', None)
            session.pop('forgot_password_email', None)
            session.pop('forgot_password_verified_at', None)
